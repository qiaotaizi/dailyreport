...
public class ReportGenerator {
	...
	private void confirmTemplateFile(String reportTemplateFile, String defaultReportTemplate) {
		File codeReviewTemplate=new File(reportTemplateFile);
		if(!codeReviewTemplate.getParentFile().exists()){
			codeReviewTemplate.getParentFile().mkdirs();
		}
		if(!codeReviewTemplate.exists()){
			FileWriter writer=null;
			try {
				codeReviewTemplate.createNewFile();
				writer=new FileWriter(codeReviewTemplate);
				writer.write(defaultReportTemplate);
				writer.flush();
			} catch (IOException e) {
				System.out.println(codeReviewTemplate.getName()+"输出失败");
				e.printStackTrace();
			}finally {
				if(writer!=null){
					try {
